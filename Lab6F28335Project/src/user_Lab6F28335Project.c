...
void main(void)
{
	...
	AdcRegs.ADCMAXCONV.all = 7;       // Convert 5 channels in SEQ1 (0-4)
	...
	AdcRegs.ADCCHSELSEQ2.bit.CONV05 = 5; // Setup ADCINA4 as 5th conversion
	AdcRegs.ADCCHSELSEQ2.bit.CONV06 = 6; // Setup ADCINA4 as 5th conversion
	AdcRegs.ADCCHSELSEQ2.bit.CONV07 = 7; // Setup ADCINA4 as 5th conversion
...
void control(void) {
	...
	/*if ((timeint - nocomm)>100) {  // if MCBSP comm stops, print to LCD screen
		if ((timeint%200)==0) {
			UART_LCDPrintfLine(1,"No Communication");
			UART_LCDPrintfLine(2,"McBSP_COMerr=%ld",McBSP_COMerr);
		}
	}*/
