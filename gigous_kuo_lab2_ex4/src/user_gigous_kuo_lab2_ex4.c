...
Step 3 of exercise 4 says the LCD should show seconds (line 2)
It does, but it's too slow. I followed directions (really, I did) and I have no idea why

running the code at step 10 DOES count every second, ON the second ("one one-thousand, two one-thousand...")

NOTE that line 590 of ColorVision.c was commented out (as per the instructions)

Maybe you can do the bold stuff on number 12. I didn't. It's 12:44am right now. I'm going home.
...
/* PRD_every1ms
 *
 * gets called every 1 millisecond
 * activates the SWI called SWI_counttime
 */
void every1ms(void)
{
...
/* SWI_counttime
 *
 * counts time in milliseconds (variable "cnt")
 * when 100ms have passed, milliseconds and seconds are print to the LCD screen
 *
 * also activates another SWI, called SWI_loaded. SWI_loaded is associated with function "stallfunc"
 *
 */
void counttime(void)
{
	...
	if ((cnt % 250) == 0)
	{
		LCDPrintfLine(1, "milliseconds %d", cnt);
		float seconds = (float)cnt/1000.0f;
		LCDPrintfLine(2, "seconds: %f", seconds);
	}

	if ((cnt % 20) == 0)
	{
		// activate stallfunc function
		SWI_post(&SWI_loaded);
	}
}

/* simulates a time load; waits until number of milliseconds has passed */
void timeload(int waittime)
{
	int starttime = CLK_getltime();
	while((CLK_getltime()-starttime)<waittime)
	{
		/* do nothing */
	}
}

/* stalls the program by a set amount in milliseconds */
void stallfunc(void)
{
	// simulate processing time of 10ms
	timeload(10);
}
