...
void RobotControl(void) {
	...
	float scaled_pwm = enc3/100.0+2.0;

	if(scaled_pwm > 15.0)
		scaled_pwm = 15.0;
	else if (scaled_pwm < 2.0)
		scaled_pwm = 2.0;

	if ((timecount % 100) == 1)
	{
		LCDPrintfLine(1,"PWM 2%% to 15%%");
		LCDPrintfLine(2,"Value: %.3f", scaled_pwm);
	}

	SetRobotOutputs(vref,turn,scaled_pwm,scaled_pwm,scaled_pwm,scaled_pwm,scaled_pwm,0,0,0);
