...
static int count=0;
static char frank[200];
static int frank_size=0;
static char carl[200];
static int carl_size=0;

char wake_up_gary = 0;
int wake_ups = 0;

/*
 *  ======== tasks and PRD's and stuff ========
 */

void millisecond_counter_raptor(void)
{
	count++;

	if ((count%250)==0){
		//LCDPrintfLine(2,"Numeric count: %d",count);
		SEM_post(&SEM_svetlana);
	}
}
	...
	while(1)
	{
		if(GET_LVDATA_FROM_LINUX==1)
		{
			...
			for (i=0;i<ptrshrdmem->DSPRec_size;i++){
				frank[i]=ptrshrdmem->DSPRec_buf[i];
			}
			frank_size=ptrshrdmem->DSPRec_size;

			carl_size=sprintf(carl,"%s count = %d",frank,count);

			CLR_LVDATA_FROM_LINUX;

			if(GET_LVDATA_TO_LINUX){
				for (i=0;i<carl_size;i++){
					ptrshrdmem->DSPSend_buf[i]=carl[i];
				}
				ptrshrdmem->DSPSend_size=carl_size;
...
/* Gary gets woken up every 250 ms */
void Gary(void)
{
	while(1)
	{
		SEM_pend(&SEM_svetlana,SYS_FOREVER);
		wake_ups++;
		LCDPrintfLine(1,"Gary woke up");
		LCDPrintfLine(2,"%d times",wake_ups);
	}
}
