...
int main (int argc, char **argv)
{
	...
	GPIO_setOutput(mygpio,VBDATA_TO_LINUX_BANK,VBDATA_TO_LINUX_FLAG,OUTPUT_HIGH);  // Set = linux is ready for VB data from DSP
	GPIO_setOutput(mygpio,VBDATA_FROM_LINUX_BANK,VBDATA_FROM_LINUX_FLAG,OUTPUT_LOW);  // Clear = linux will write VB data for shared memory for DSP
	...
	if (inputdone == 2) {  // Start TCPIP connection with VB application    
...
int run_server(void)
{
	...
	while (!gs_quit) {  // sit inside this while checking for new data from the DSP until TCPIP connection disconnected
		...
		if  (GPIO_getOutput(mygpio,VBDATA_TO_LINUX_BANK,VBDATA_TO_LINUX_FLAG) == 0) { // New data from DSP ?    
			...
			tmpbuf[i] = '\0';
			numTXChars = sprintf(((char *) outbuf),"%c%s%c",253,tmpbuf,255);  // create string to be sent to VB with added start and stop character.  
			write(accepted_skt, outbuf, numTXChars);  // write string to TCPIP socket, VB waiting on the other end.

			printf("%s\n",tmpbuf);
			
			GPIO_setOutput(mygpio,VBDATA_TO_LINUX_BANK,VBDATA_TO_LINUX_FLAG,OUTPUT_HIGH); // set flag indicating to DSP that Linux is ready for more data
		}
		...
		for (i=0;i<numrecChars;i++) {  // loop through all the recieved characters
			if (!TCPIPbeginnewdata) {// Only true if have not yet begun a message
				...
				if ((TCPIPdatasize < 256) && ((unsigned char)inbuf[i] != 255)) {  // also check that it is not stop character	
					...
					if (GPIO_getOutput(mygpio,VBDATA_FROM_LINUX_BANK,VBDATA_FROM_LINUX_FLAG)  == 0) {  // is DSP ready for new data?
						...
						GPIO_setOutput(mygpio,VBDATA_FROM_LINUX_BANK,VBDATA_FROM_LINUX_FLAG,OUTPUT_HIGH);
					}
