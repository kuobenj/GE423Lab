...
/* CORNER DETECTION */
int corner = 0;
typedef enum {NORTH, SOUTH, EAST, WEST, NOPE} direction;
direction compass_dir = NORTH;
...
int cen_y_shr;
int new_print_shr;
unsigned char ball_toggle_shr;
unsigned char found_wall_shr;
unsigned char found_ball_shr;

float Kp_vision = 0.05;
float vision_set = 0.0;

int cen_x_robot_ctrl = 0;
int cen_y_robot_ctrl = 0;
int wall_x_robot_ctrl = 0;
int wall_y_robot_ctrl = 0;
unsigned char ball_toggle_robot_ctrl = 0;
unsigned int found_wall_robot_ctrl = 0;
unsigned char found_ball_robot_ctrl = 0;
/**/

unsigned int dummy = 0;

unsigned int robot_reset_time = 0;
...
void RobotControl(void) {
	...
	if (new_print_shr)
	{
		ball_toggle_robot_ctrl = ball_toggle_shr;

		if (ball_toggle_robot_ctrl == BLUE_ON)
		{
			cen_x_robot_ctrl = cen_x_shr;
			cen_y_robot_ctrl = cen_y_shr;
			found_ball_robot_ctrl = found_ball_shr;
		}
		else if (ball_toggle_robot_ctrl == GREEN_ON)
		{
			wall_x_robot_ctrl = cen_x_shr;
			wall_y_robot_ctrl = cen_y_shr;
			found_wall_robot_ctrl = found_wall_shr;
		}
	...
	if (front_wall_error > front_error_threshold){
		...
		//DUMMY NOW USED FOR RESETING OF POSE
		//dummy += 1; 

		if (right_wall_error < 100 || right_wall_error > -100)
		{
			corner = 1;
		}
	}
	...
	/* COMPASS DIRECTION CODE STUFF */
	if (compass > 100 && compass < 400)
		compass_dir = NORTH;
	else if (compass > 1900 && compass < 2100)
		compass_dir = WEST;
	else if (compass > 1600 && compass < 1800)
		compass_dir = SOUTH;
	else if (compass > 1000 && compass < 1200)
		compass_dir = EAST;
	else
		compass_dir = NOPE;

	if (corner && found_wall_robot_ctrl && (timecount > (robot_reset_time + 3000)))
	{
		if (compass_dir == NORTH)
		{
			pos_x = 5;
			pos_y = 10;
			angle_integral_amp = 3.14159/2.0;
			angle_integral = 3.14159/2.0;
			dummy++;
			robot_reset_time = timecount;
		}
		else if(compass_dir == WEST)
		{
			pos_x = -5;
			pos_y = 10;
			angle_integral_amp = 3.14159;
			angle_integral = 3.14159;
			dummy++;
			robot_reset_time = timecount;
		}
	}
	...
	if ((timecount % 100) == 1)
	{
		switch ((int) switchstate)
		{
			...
			case 4: 
				...
				break;
			case 5:
				LCDPrintfLine(1,"cen_x: %d", cen_x_robot_ctrl);
				LCDPrintfLine(2,"cen_y: %d", cen_y_robot_ctrl);
				// LCDPrintfLine(2,"time: %d", timecount);
				break;
			case 6:
				LCDPrintfLine(1,"cnr: %d", corner);
				switch (compass_dir)
				{
					case NORTH:
						LCDPrintfLine(2,"compass: NORTH", compass_dir);
						break;
					case SOUTH:
						LCDPrintfLine(2,"compass: SOUTH", compass_dir);
						break;
					case EAST:
						LCDPrintfLine(2,"compass: EAST", compass_dir);
						break;
					case WEST:
						LCDPrintfLine(2,"compass: WEST", compass_dir);
						break;
					default:
						LCDPrintfLine(2,"compass: INVALID", compass_dir);
						break;
				}
				//LCDPrintfLine(2,"rwe: %f", right_wall_error);
				break;
			case 7:
			...
			case 12:
				...
				LCDPrintfLine(2,"wall: %d", found_wall_robot_ctrl);
				break;
			case 13:
