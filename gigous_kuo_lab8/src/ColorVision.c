...
#define GREEN_HRAD 			8
#define GREEN_S 			161
#define GREEN_SRAD 			22
#define GREEN_V 			190
#define GREEN_VRAD 			64

#define BLUE_ON				1
#define GREEN_ON			0

unsigned char ball_toggle = 0;
unsigned char found_wall = 0;
unsigned char found_ball = 0;

//shared vars
extern unsigned char ball_toggle_shr;
extern unsigned char found_wall_shr;
extern unsigned char found_ball_shr;
extern int cen_x_shr;
extern int cen_y_shr;
extern int new_print_shr;
/* END OUR COLOR VISION STUFF */
...
void init_ColorVision(void) {
	...
	//for blue ball without ir lights


	specs_h = BLUE_H;
	specs_hrad = BLUE_HRAD;
	if((specs_h-specs_hrad)<0) // wrap 0->360
	{
		specs_h2=specs_h+256;
	}
	...
	specs_s = BLUE_S;
	specs_srad = BLUE_SRAD;
	specs_v = BLUE_V;
	specs_vrad = BLUE_VRAD;
...
void userProcessColorImageFunc_laser(bgr *ptrImage) {
	...
	if (ptrImage != NULL) {
		...
		if (ball_toggle)
		{
			...
			ball_toggle = GREEN_ON;
		}
		else
		{
			specs_h = GREEN_H;
			specs_hrad = GREEN_HRAD;
			if((specs_h-specs_hrad)<0) // wrap 0->360
			{
				specs_h2=specs_h+256;
			}
			...
			specs_s = GREEN_S;
			specs_srad = GREEN_SRAD;
			specs_v = GREEN_V;
			specs_vrad = GREEN_VRAD;
...
//            }
		...
		if (new_coordata == 0) {
			if (final_object_stats[largest_object].num_pixels_in_object > 7) {
				noimagefound = 0;
				...
				if (ball_toggle == BLUE_ON)
					found_wall = 1;
				else if (ball_toggle == GREEN_ON)
					found_ball = 1;
			...
			//#######################################//
			/* THIS IS WHERE WE SET SHARED VARIABLES */
			//#######################################//
			ball_toggle_shr = ball_toggle;
			cen_x_shr = (int) object_x;
			cen_y_shr = (int) object_y;
			found_wall_shr = found_wall;
			found_ball_shr = found_ball;
			new_print_shr = new_coordata;
		}
