...
#define pixel(x,y) 		ptrImage[(y)*IMAGE_COLUMNS+(x)]
#define CUTOFF 			60
...
void userProcessColorImageFunc_laser(bgr *ptrImage) {
	...
	if (ptrImage != NULL) {
		...
		int y, x;
		for (y = 0; y < IMAGE_ROWS - CUTOFF; y++)
		{
			for (x = 0; x < IMAGE_COLUMNS; x++)
			{
				// Remember we're starting in bottom right
				if ((pixel(x,y).blue > 230) && (pixel(x,y).green > 230) && (pixel(x,y).red > 230))
				{
					row_sum += y;
					col_sum += x;
					num_pix++;
		...
		if (new_print_shr == 0)
		{
			if (!(num_pix < 25))
			{
				cen_y = row_sum / num_pix; // the centroid of color blobs
				cen_x = col_sum / num_pix;
			}
			if (
					(cen_x >= (IMAGE_COLUMNS - 1))
					|| 	(cen_x < 2)
					||	(cen_y >= (IMAGE_ROWS - 1))
					||	(cen_y < 2)
			)
			...
			{
				pixel(cen_x, cen_y).blue = 0;
				pixel(cen_x, cen_y).green = 255;
				pixel(cen_x, cen_y).red = 0;

				pixel(cen_x, cen_y).blue = 0;
				pixel(cen_x + 1, cen_y).green = 255;
				pixel(cen_x + 1, cen_y).red = 0;
				...
				pixel(cen_x, cen_y).blue = 0;
				pixel(cen_x, cen_y + 1).green = 255;
				pixel(cen_x, cen_y + 1).red = 0;
			...
			cen_x_shr = cen_x-(IMAGE_COLUMNS/2);
			cen_y_shr = cen_y-(IMAGE_ROWS/2);
			new_print_shr = 1;
		}

		// END VISION PROCESSING CODE STUFF
